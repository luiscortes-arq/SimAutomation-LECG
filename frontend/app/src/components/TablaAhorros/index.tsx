...
import { Clock, Zap, TrendingUp, DollarSign, Timer, Activity } from 'lucide-react';
import {
  AreaChart,
  Area,
  XAxis,
...
export const TablaAhorros: React.FC = () => {
  ...
  const COLORS = {
    manual: '#C42021',   // Warm Red (Current Process)
    invest: '#6C7D47',   // Palm (Investment)
    auto: '#254467',     // Dark Blue (Automation Operations)
    text: '#67727A',     // Gray
    grid: '#333333',
    gold: '#EFB628',
    cyan: '#06b6d4'      // Highlight for tech feel
  };
  ...
  // --- CONSTANTS ---
  const WEEKS = 52;
  const MANUAL_WEEKLY_COST = 8640; 
  const AUTO_WEEKLY_COST = 2880;   
  
  const INVESTMENT_START = 34560;
  const INVESTMENT_ADDITION = 138240;
  const INVESTMENT_TOTAL_PHASE = INVESTMENT_START + INVESTMENT_ADDITION; 
  
  const AUTO_START_WEEK = 8; 

  // Throughput Rates (Simulations per week)
  const RATE_MANUAL = 1;
  const RATE_AUTO = 3;
  ...
  const costData = useMemo(() => {
    ...
    for (let i = 0; i <= WEEKS; i++) { 
      ...
      weeklyData.push({
        week: week,
        label: `S${week}`, // Changed to 'S' for Semana
        costManual,
        costAuto,
      });
    }
            ...
            {payload.map((entry: any, idx: number) => (
                <div key={idx} className="flex justify-between items-center gap-4">
                    <span className="text-[10px] uppercase text-zinc-400 font-bold flex items-center gap-2">
                        ...
                        {entry.name === 'costAuto' ? 'Automatizado' : entry.name === 'costManual' ? 'Manual' : entry.name === 'autoTotal' ? 'Velocidad Auto' : 'Velocidad Manual'}
                    </span>
  ...
  return (
    <section className="h-full w-full bg-zinc-50 dark:bg-[#020202] flex flex-col items-center justify-start py-8 relative overflow-y-auto transition-colors duration-500 scroll-smooth">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end w-full relative">
            <div className="text-left mb-6 md:mb-0 max-w-xl z-10">
                ...
                <h2 className="text-4xl md:text-6xl font-black text-hermosillo-darkBlue dark:text-white tracking-tighter mb-2">
                    ANÁLISIS DE <span className="text-hermosillo-red">VALOR</span>
                </h2>
            </div>

            <div className="flex justify-start md:justify-end w-full md:w-auto">
                <div className="bg-white/90 dark:bg-[#0a0f16]/90 backdrop-blur-xl border border-zinc-200 dark:border-zinc-800 p-1 rounded-full flex gap-1 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
                    <button
                        onClick={() => setActiveTab('cost')}
                        className={`relative z-10 px-6 py-2.5 rounded-full text-[10px] font-bold uppercase tracking-[0.15em] transition-all duration-300 flex items-center gap-2 cursor-pointer ${
                            activeTab === 'cost' 
                            ? 'text-white' 
            ...
            {activeTab === 'cost' ? (
                ...
                >
                    ...
                    <div className="relative group">
                        ...
                        <div className="absolute top-0 left-0 right-0 h-10 bg-white/5 dark:bg-white/5 border-b border-zinc-200 dark:border-white/5 flex justify-between items-center px-4 z-20 rounded-t-xl">
                            <div className="flex items-center gap-3">
                                <span className="text-[10px] font-mono text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">Vista_Proyección_01</span>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="flex items-center gap-2">
                                    <div className="w-1.5 h-1.5 rounded-full bg-hermosillo-red"></div>
                                    <span className="text-[9px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Manual</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-1.5 h-1.5 rounded-full bg-hermosillo-darkBlue shadow-[0_0_8px_#254467]"></div>
                                    <span className="text-[9px] font-bold text-hermosillo-darkBlue dark:text-white uppercase tracking-wider">Auto</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        <div className="bg-white dark:bg-[#070707] rounded-xl border border-zinc-200 dark:border-white/10 overflow-hidden shadow-sm">
                             {/* Header */}
                             ...
                             <div className="divide-y divide-zinc-100 dark:divide-white/5">
                                 ...
                                 <div className="grid grid-cols-12 py-4 px-6 items-center hover:bg-zinc-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                     <div className="col-span-4 flex flex-col">
                                         <span className="text-sm font-bold text-zinc-700 dark:text-zinc-200">Recursos Humanos</span>
                                         <span className="text-[10px] text-zinc-400">Personal asignado</span>
                                     </div>
                                     <div className="col-span-3 text-right font-mono text-xs text-zinc-600 dark:text-zinc-400">1 Persona</div>
                                     <div className="col-span-2 text-right font-mono text-xs text-zinc-600 dark:text-zinc-400">2 Personas</div>
                                     <div className="col-span-3 text-right font-mono text-xs font-bold text-hermosillo-darkBlue dark:text-white">1 Persona</div>
                                 </div>

                                 {/* Row 2 */}
                                 <div className="grid grid-cols-12 py-4 px-6 items-center hover:bg-zinc-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                     <div className="col-span-4 flex flex-col">
                                         <span className="text-sm font-bold text-zinc-700 dark:text-zinc-200">Costo Operativo</span>
                                         <span className="text-[10px] text-zinc-400">Semanal</span>
                                     </div>
                ...
                >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                            {/* KPI 1 */}
                            <div className="bg-white dark:bg-[#070707] border border-zinc-200 dark:border-white/10 p-6 rounded-xl flex flex-col justify-between hover:border-hermosillo-red/50 transition-colors">
                                <div className="flex justify-between items-start mb-2">
                                    <span className="text-[9px] uppercase font-bold text-zinc-400 tracking-widest">Tiempo Manual</span>
                                ...
                                <div>
                                    ...
                                    <span className="text-[10px] text-zinc-400 uppercase">Semanas requeridas</span>
                                </div>
                            ...
                            {/* KPI 2 */}
                            <div className="bg-white dark:bg-[#070707] border border-zinc-200 dark:border-white/10 p-6 rounded-xl flex flex-col justify-between hover:border-hermosillo-palm/50 transition-colors relative overflow-hidden">
                                <div className="flex justify-between items-start mb-2">
                                    <span className="text-[9px] uppercase font-bold text-hermosillo-palm tracking-widest">Tiempo Auto</span>
                                    <Zap size={14} className="text-hermosillo-palm" />
                                </div>
                                <div>
                    ...
                    <div className="relative group">
                         ...
                         <div className="absolute top-0 left-0 right-0 h-10 bg-white/5 dark:bg-white/5 border-b border-zinc-200 dark:border-white/5 flex justify-between items-center px-4 z-20 rounded-t-xl">
                            <div className="flex items-center gap-3">
                                <span className="text-[10px] font-mono text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">Análisis_Velocidad_V2</span>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="flex items-center gap-2">
                                    <div className="w-1.5 h-1.5 rounded-sm bg-hermosillo-red"></div>
                                    <span className="text-[9px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Pendiente Manual</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-1.5 h-1.5 rounded-full bg-hermosillo-auto"></div>
                                    <span className="text-[9px] font-bold text-hermosillo-darkBlue dark:text-white uppercase tracking-wider">Velocidad Auto</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={productivityData} margin={{ top: 20, right: 30, left: 10, bottom: 10 }}>
                                    ...
                                    <ReferenceLine 
                                        ...
                                        strokeDasharray="4 4" 
                                        strokeWidth={1}
                                        label={{ value: 'META OBJETIVO', position: 'insideTopRight', fill: COLORS.gold, fontSize: 9, fontFamily: 'monospace' }} 
                                    />
                                    ...
                                    <Line 
                                        ...
                                        strokeWidth={3} 
                                        dot={{ r: 0 }}
                                        activeDot={{ r: 6, fill: COLORS.auto, stroke: '#fff', strokeWidth: 2 }}
                                    />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
